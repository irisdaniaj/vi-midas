...
// here we have implemented the model estimation with holdout index for training and testing  
    ...
    int d; // number of satellite indicators 
    ...
    row_vector[d] D[n]; // observed satellite data  
...
parameters{
    ...
    matrix[p,l] A_geo;    // latent variable for geo-chemical factors 
...
model {
  ...
  to_vector(A_geo)   ~  double_exponential(0, sp_mean); //sparse 
  to_vector(A_d) ~  double_exponential(0, sp_mean); 
  phi   ~  normal(0, sp_var);
  ...
  for(i in 1:n){
    ...
    for(j in 1:q){
        if (holdout[i,j] == 0){
            ...
            mu[j] = C0[j] + X[i]*(A_geo*(L_sp[j,]')) + S[i]*(A_s*(L_sp[j,]')) + B[i]*(A_b*(L_sp[j,]')) + Q[i]*(A_m*(L_sp[j,]')) + D[i]*(A_d*(L_sp[j,]'));
            if (Yi[i,j] != 0){ 
