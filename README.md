...
```
sbatch mem_hyperparameter_tuning
```
The `mem_hyperparameter_tuning` script uses Slurm to perform parallel hyperparameter tuning by running tasks as a job array. Each task reads hyperparameters from `results/hyperparameter/hyperparams.csv` and executes the `hyperparameter_tuning_fit.py` script with specified values. You may need to adjust specific cluster-specific configuration in `mem_hyperparameter_tuning` (e.g., `#SBATCH` options). The results will be saved in `results/hyperparameter`. 
